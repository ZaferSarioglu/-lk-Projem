...
class PHPMailer{
    ...
    public $ErrorInfo = '';
    public $From = 'info@siteadresi.com';
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        $PHPMAILER_LANG = array(
            'authenticate' => 'SMTP Hatası: kimlik doğrulaması oluşturulamadı.',
            'connect_host' => 'SMTP Hatası: SMTP ana makineye bağlanılamadı.',
            'data_not_accepted' => 'SMTP Hatası: veri kabul edilmiyor.',
            'empty_message' => 'Mesaj içeriği boş gönderilemez',
            'encoding' => 'Bilinmeyen kodlama: ',
            'execute' => 'Çalıştırılamadı: ',
            'file_access' => 'Dosyaya erişilemedi: ',
            'file_open' => 'Dosya Hatası: Dosya açılamadı: ',
            'from_failed' => 'Aşağıdaki Kimden adresi başarısız oldu: ',
            'instantiate' => 'Mail gönderme fonksiyonu başlatılamadı.',
            'invalid_address' => 'Geçersiz adres: ',
            'mailer_not_supported' => ' mailler desteklenmiyor.',
            'provide_address' => 'En az bir alıcı mail adresi girmelisiniz.',
            'recipients_failed' => 'SMTP Hatası: Aşağıdaki alıcılar başarısız oldu: ',
            'signing' => 'İmza hatası: ',
            'smtp_connect_failed' => 'SMTP hosta bağlanılamadı.',
            'smtp_error' => 'SMTP sunucu hatası: ',
            'variable_set' => 'Değişkenler tanımlanamıyor veya silinemiyor: ',
            'extension_missing' => 'Uzantı eksik: '
        );
        if (empty($lang_path)) {
...
class SMTP{
    ...
    public function connected()
    {
        if (is_resource($this->smtp_conn)) {
            ...
            if ($sock_status['eof']) {
                $this->edebug(
                    'SMTP Uyarı: EOF Bağlantı olup olmadığını kontrol ediliyor',
                    self::DEBUG_CLIENT
                );
                $this->close();
                return false;
